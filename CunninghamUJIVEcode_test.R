pacman::p_load(pracma, foreach, data.table)

set.seed(10)

ss = 100

test = data.table(Y = runif(ss), X = runif(ss), Z = runif(ss))

fwrite(test, "C:/Users/Nathan/Downloads/Duflo 2001 Stuff/ivexample.csv")

throwaway = foreach(i = 1:ss) %do% {
  test[i, xhat := predict(lm(X ~ Z, data = test[!i]), newdata = test[i])]
  NULL
}
test[, xhat_no_loo := predict(lm(X ~ Z))]
summary(lm(Y ~ X, data = test))
summary(lm(Y ~ xhat, data = test))
summary(lm(Y ~ xhat_no_loo, data = test))





ivreg_cunningham <- function (y, T, Z, W, noConstant = TRUE) {
...
ivreg_cunningham(test$Y, test$X, test$Z, rep(1, nrow(test)), noConstant = FALSE)

xtildemat = cbind(test$xhat, rep(1, nrow(test)))

solve(t(xtildemat) %*% xtildemat) %*% t(xtildemat) %*% test$Y

solve(t(xtildemat) %*% cbind(test$X, rep(1, nrow(test)))) %*% t(xtildemat) %*% test$Y
