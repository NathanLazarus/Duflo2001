pacman::p_load(pracma, tidyverse, fastDummies)
...
test = cbind(
  data.table(dummy_cols(asdf[young == 1 | old == 1]$birthyr, remove_selected_columns = T)),
  data.table(dummy_cols(asdf[young == 1 | old == 1]$birthpl, remove_selected_columns = T)),
  data.table(as.matrix(dummy_cols(asdf[young == 1 | old == 1]$birthyr, remove_selected_columns = T)) * asdf[young == 1 | old == 1]$child_pop_71)
)

stats  <- ivreg(
  asdf[young == 1 | old == 1]$lhwage,
  asdf[young == 1 | old == 1]$yeduc,
  asdf[young == 1 | old == 1]$young * asdf[young == 1 | old == 1]$n_inpres_schools,
  test
)
